...
export default createFunc(
  {
    "nixpkgs": (nixScope) => (new Path(["../../.."], [])),
    "system": (nixScope) => (nixScope.builtins["currentSystem"]),
    "pkgs": (
      nixScope,
    ) => (nixScope.import(nixScope.nixpkgs)(
      { "config": {}, "overlays": [], "system": nixScope.system },
    )),
    "nixVersions": (
      nixScope,
    ) => (nixScope.import(new Path(["../../tests/nix-for-tests.nix"], []))(
      { "pkgs": nixScope.pkgs },
    )),
    "libpath": (nixScope) => (new Path(["../.."], [])),
    "seed": (nixScope) => (null),
  },
  null,
  {},
  (nixScope) => (
    nixScope.pkgs["runCommand"]("lib-path-tests")(
      {
        "nativeBuildInputs": operators.listConcat(
          [nixScope.nixVersions["stable"]],
          ((_withAttrs) => {
            const nixScope = {
