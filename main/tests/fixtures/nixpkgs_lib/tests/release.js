...
//    ,
//}
createFunc({"system": (nixScope)=>(nixScope["builtins"]["currentSystem"]),"pkgs": (nixScope)=>(operators.merge(nixScope["import"]((new Path(["../.."], [])))(({"system": nixScope["system"], "config": ({"permittedInsecurePackages": ["nix-2.3.18"]})})), ({"lib": nixScope["throw"]("pkgs.lib accessed, but the lib tests should use nixpkgs' lib path directly!")}))),"pkgsBB": (nixScope)=>(nixScope["pkgs"]["pkgsBuildBuild"]),"nix": (nixScope)=>(nixScope["pkgs-nixVersions"]["stable"]),"nixVersions": (nixScope)=>([nixScope["pkgs-nixVersions"]["minimum"],nixScope["nix"],nixScope["pkgs-nixVersions"]["latest"]]),"pkgs-nixVersions": (nixScope)=>(nixScope["import"]((new Path(["./nix-for-tests.nix"], [])))(({"pkgs": nixScope["pkgsBB"]}))),}, null, {}, (nixScope)=>(
                (function(){
