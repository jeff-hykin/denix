...
// Auto-generated from fetchGit.sh:127
// Dirty working tree should report dirtyShortRev as shortrev-dirty
...
await withGitRepo({
    ...
    const res = await runDenix(["--eval", "--impure", "--raw", "--expr",
        `(builtins.fetchGit ${repoDir}).dirtyShortRev`])
    const actual = (res.stdout || "").replace(/\n+$/, "")
    const expected = `${revs[1].substring(0, 7)}-dirty`
    if (actual !== expected || res.code !== 0) {
        console.error("FAIL: dirtyShortRev", { actual, expected, stderr: res.stderr })
        Deno.exit(1)
    }
})
