...
// Auto-generated from fetchGit.sh:143
// After committing, hasAttr "dirtyRev" should be false
...
await withGitRepo({
    ...
    const res = await runDenix(["--eval", "--impure", "--expr",
        `builtins.hasAttr "dirtyRev" (builtins.fetchGit ${repoDir})`])
    const actual = (res.stdout || "").replace(/\n+$/, "")
    if (actual !== "false" || res.code !== 0) {
        console.error("FAIL: hasAttr dirtyRev", { actual, expected: "false", stderr: res.stderr })
        Deno.exit(1)
    }
})
