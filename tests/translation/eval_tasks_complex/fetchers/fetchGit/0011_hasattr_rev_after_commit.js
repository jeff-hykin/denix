...
// Auto-generated from fetchGit.sh:142
// After committing dirty tree, hasAttr "rev" should be true
...
await withGitRepo({
    ...
    // Make dirty then commit (simulating the script's flow at lines 110-137)
    ...
    const res = await runDenix(["--eval", "--impure", "--expr",
        `builtins.hasAttr "rev" (builtins.fetchGit ${repoDir})`])
    const actual = (res.stdout || "").replace(/\n+$/, "")
    if (actual !== "true" || res.code !== 0) {
        console.error("FAIL: hasAttr rev", { actual, expected: "true", stderr: res.stderr })
        Deno.exit(1)
    }
})
