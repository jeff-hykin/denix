...
// Auto-generated from fetchGit.sh:225-227
...
await withGitRepo({
    ...
    const headRev = await git(repoDir, "rev-parse", "HEAD")

    const res = await runDenix(["--eval", "--impure", "--raw", "--expr",
        `(builtins.fetchGit { url = "file://${repoDir}"; ref = "HEAD"; }).rev`])
    const actual = (res.stdout || "").replace(/\n+$/, "")
    if (actual !== headRev || res.code !== 0) {
        console.error("FAIL: ref HEAD rev", { actual, expected: headRev, stderr: res.stderr })
        Deno.exit(1)
    }
})
