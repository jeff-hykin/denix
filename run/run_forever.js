...
while (true) {
    ...
    try {
        ...
        await $`claude --allowedTools "Task,Edit,Read,Update,Write,WebFetch,WebSearch,Glob,Grep,Write(./**/*),Bash(*)" -p ${`
                ...
                1. Understand the codebase goals (make runtime.js work)
                ...
                There should be a simple testing system that tests js behavior against nix behavior. Create and run tests yourself. There should be a way to run all tests, run groups of tests, or a run test on a specific aspect (like all tests for "derivation")
    ...
    try {
        ...
        await $`claude --allowedTools "Task,Edit,Read,Update,Write,WebFetch,WebSearch,Glob,Grep,Write(./**/*),Bash(*)" -p ${`
                ...
                - Make sure this is always at the top:
                    Your job is to focus on writing code and implementing what is NOT implemented and NOT working. Only report what remains to be done. Do not report what you accomplished. You are a senior level developer, there is no such thing as a blocker. Break down large tasks into smaller tasks.
