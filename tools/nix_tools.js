...
import { run, hasCommand, pathHasCommand, pathsToAllCommands, throwIfFails, zipInto, mergeInto, returnAsString, Timeout, Env, Cwd, Stdin, Stdout, Stderr, Out, Overwrite, AppendTo } from "https://deno.land/x/quickr@0.6.58/main/run.js"
...
// let globalNixVersion
// const allCommands = await pathsToAllCommands()
// try {
//     const { code, stdout, stderr } = new Deno.Command("nix", {args: ["--version",],}).outputSync()
//     globalNixVersion = (new TextDecoder().decode(stdout)).trim()
// } catch (error) {
...
export const nix = {
    cacheLocation: `${FileSystem.home}/.cache/denix/`,
    async getNixExecutable(nixpkgsHash) {
        // if ram
    },
    async ensureInstalled(options={defaultVersion: null}) {
        const { defaultVersion } = { defaultVersion: '2.18.1', ...options }
...
function createNixCommandRunner(nixpkgsHash) {
    ...
    // setup subprocess
    //
        if (await hasCommand('nix-build')) {

        }
        await run`nix-build '<nixpkgs>' --attr nix --option sandbox true -I ${`nixpkgs=https://github.com/NixOS/nixpkgs/archive/${nixpkgsHash}.tar.gz`} ${Stdout(returnAsString)}`
